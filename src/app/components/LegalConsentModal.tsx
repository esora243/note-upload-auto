 ...
 *   「同意して始める」ボタンが有効化されない
 * - デザインは既存 LoginModal / Layout のカラーパレット (#11204C / #1E3A8A / #B9C2DB) に準拠
 */
export function LegalConsentModal({ isOpen, onAgree }: LegalConsentModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[200] flex items-center justify-center p-3 sm:p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200"
      role="dialog"
      aria-modal="true"
      aria-labelledby="legal-consent-title"
    >
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
        {/* Header */}
        <div
        ...
        <div className="relative flex-1 min-h-0 bg-[#F8FAFD]">
          ...
          {/* Scroll hint (until current tab is read) */}
...
function ScrollPanel({
  ...
  // 見えていないときも DOM は保持しておく (スクロール状態を維持するため)
  const style = useMemo(
